...
class WebUSBBridge(QObject):
    ...
    def __init__(self, browser_window=None, parent=None,
        ...
        # 🛡️ security_audit No.2: requestDeviceChooser()の「本物のユーザー操作
        #    (クリック等)からの呼び出しか」をPython側で検証するためのトークン
        #    ストア。polyfill.py がnavigator.userActivation.isActiveを確認した
        #    直後にmintGestureToken()を呼んで発行を受け、そのトークンを
        #    requestDeviceChooser()へ渡す。トークンは短命(_GESTURE_TOKEN_TTL_SECONDS)
        #    ・使い切り(検証時にpopする)。
        #    ⚠️ 正直な限界: mintGestureToken()自体も他の@Slotと同様、素のQWebChannel
        #    接続から直接叩ける。PySide6/QtWebEngineの公開APIには「ページ内で
        #    本物のユーザー操作が今まさに起きているか」をPython側から独立に
        #    確認する手段が無いため、この仕組みは「polyfill.py経由の正規フローが
        #    最低限もう1往復のハンドシェイクを踏まなければならない」という
        #    フリクションを課すこと、およびNo.2bのフィルタ構造検証を確実に
        #    サーバー側で通すことまでは達成できるが、mint自体を直接叩く
        #    高度に自動化された敵対的スクリプトまでは防げない。実ブラウザ相当の
        #    完全な保証には、Chromiumの実際のUser Activation状態をQt側から
        #    観測できるAPIが必要で、現時点のPySide6/QtWebEngineには存在しない。
        self._gesture_tokens = {}  # token(str) -> 発行時刻(time.monotonic())
    ...
    def isGrantedToThisFrame(self, vendor_id, product_id, frame_token=""):
        """🛡️ security_audit No.5: このオリジン(=呼び出し元frame_tokenが指す
        フレーム自身)が(vendorId, productId)への許可を実際に持っているかだけを
        answerする、副作用の無い軽量な問い合わせ。listDevices()と違い
        usb_core.find(find_all=True)で現在の物理接続状態を見る必要が無いため、
        「今まさに切断されたデバイス」についても正しく判定できる
        (listDevices()は接続中のデバイスしか列挙しないため、切断イベントの
        検証に使うとdisconnectイベントを全て握りつぶしてしまう)。
        polyfill.py側は、Python発のdeviceConnected/deviceDisconnected
        シグナル(QWebChannelの仕組み上ページ内の全フレームへブロードキャスト
        される。_poll_hotplug()のコメント参照)を実際にページへdispatchする前に、
        これを使って「本当に自分自身のオリジンが許可されているデバイスか」を
        再検証する。ブロードキャスト自体はQt Signalの都合上フレーム単位に
        絞れないが、observableな挙動としては無関係なフレームに漏れないように
        できる。listDevices()と全く同じ_is_granted()判定を、例外時は安全側
        (False)に倒しつつ流用する。"""
        try:
            origin = self._current_origin(frame_token)
            if not origin:
                return False
            return bool(self._is_granted(origin, vendor_id, product_id))
        except Exception:
            return False
    ...
    _GESTURE_TOKEN_CACHE_CAP = 64     # 単一ページが妥当な理由で同時に持つ発行済みトークン数を大きく超える上限
    _MAX_OPEN_HANDLES_PER_ORIGIN = 64  # security_audit No.6: 正当なユースケースでも十分すぎるほど余裕を持たせた上限

    @Slot(result=str)
    def mintGestureToken(self):
        """polyfill.pyがnavigator.userActivation.isActiveを確認した直後に呼ぶ。
        短命・使い切りの不可予測トークンを発行し、requestDeviceChooser()への
        次の呼び出しがこの発行と対になっていることを検証できるようにする
        (see: __init__のself._gesture_tokensのコメント、およびNo.2の限界)。"""
        now = time.monotonic()
        # 失効済みトークンを掃除してから上限を適用する(古いものから捨てる)
        for tok in [t for t, minted_at in self._gesture_tokens.items() if now - minted_at > self._GESTURE_TOKEN_TTL_SECONDS]:
            self._gesture_tokens.pop(tok, None)
        while len(self._gesture_tokens) >= self._GESTURE_TOKEN_CACHE_CAP:
            oldest = min(self._gesture_tokens, key=self._gesture_tokens.get)
            self._gesture_tokens.pop(oldest, None)
        token = secrets.token_urlsafe(24)
        self._gesture_tokens[token] = now
        return token

    def _consume_gesture_token(self, token):
        """有効(未失効・未使用)なトークンなら消費してTrueを返す。使い切りに
        することで、同じトークンを使い回した複数回のrequestDeviceChooser()呼び出しを防ぐ。"""
        if not token or not isinstance(token, str):
            return False
        minted_at = self._gesture_tokens.pop(token, None)
        if minted_at is None:
            return False
        return (time.monotonic() - minted_at) <= self._GESTURE_TOKEN_TTL_SECONDS

    @Slot(str, str, str, result=str)
    def requestDeviceChooser(self, options_json, frame_token="", gesture_token=""):
        ...
        🛡️ security_audit No.2: gesture_token — mintGestureToken()が発行した
        トークンをここで検証・消費する。欠落・失効・使い切り済みならダイアログを
        一切構築せずSecurityErrorで拒否する(=素のQWebChannel直叩きで
        gesture_tokenを渡す方法を知らない/持たない敵対的スクリプトは、
        ここで必ず弾かれる)。mintGestureToken自体の限界はそのdocstring/
        __init__のコメントを参照。
        チェック順序: 再入防止ガード(_chooser_active)を先に見る — 既にダイアログが
        開いている場合はInvalidStateErrorの方がgesture_token不備によるSecurityError
        よりも呼び出し元にとって直接的で分かりやすい理由になるため。"""
        if self._chooser_active:
            return _json_dumps({
                "cancelled": True,
                "error": invalid_state_error("a device chooser is already open for this page"),
            })
        if not self._consume_gesture_token(gesture_token):
            return _json_dumps({
                "cancelled": True,
                "error": security_error("Must be handling a user gesture to call navigator.usb.requestDevice()."),
            })
    ...
    def _request_device_chooser_impl(self, options_json, frame_token=""):
        ...
        try:
            ...
            # 🛡️ security_audit No.2b: filters/exclusionFiltersの構造妥当性
            # ("A USBDeviceFilter filter is valid")はこれまでpolyfill.pyの
            # JS側だけで検証しており、Python側はここまで「構造的に妥当な
            # ものだけが渡ってくる」という前提に完全に依存していた
            # (このメソッド自身のdocstringが明記していた前提)。素の
            # QWebChannel接続からrequestDeviceChooserを直接叩けば、この
            # 前提はJS側のチェックごと丸ごと素通りする。実ブラウザは無効な
            # filterに対してTypeErrorで(チューザーを開く前に)同期的に拒否
            # するため、ここでも同じタイミングで同じ強さの拒否を行う
            # (中途半端に「一致件数を絞る」のではなく、要求そのものを拒否する)。
            invalid_filters = [f for f in (filters + exclusion_filters) if not is_valid_usb_device_filter(f)]
            if invalid_filters:
                return _json_dumps({
                    "cancelled": True,
                    "error": type_error(
    ...
    def openDevice(self, vendor_id, product_id, frame_token=""):
        ...
        🛡️ security_audit No.6: 1オリジンが同時に保持できるハンドル数に上限
        (_MAX_OPEN_HANDLES_PER_ORIGIN)を設ける。旧実装はこれが無制限で、
        たった1台のデバイスへの正規の許可さえあれば、closeDevice()を挟まず
        device.open()をループで呼び続けるだけの、素のQWebChannel直叩きすら
        不要なごく普通のJSコードで、_open_devices(とその中のpyusb Deviceへの
        参照)を無制限に増やし続けられた。WebUSBBridgeはホストアプリ本体の
        プロセス内で動くため、これは表示中のタブのメモリではなくホスト
        アプリケーション本体のメモリを消費する(=通常のレンダラプロセス分離や
        タブ単位のメモリ上限では守られない)。
        方式はエラーでの拒否ではなくLRU的な自動退去: 上限に達している状態で
        新規にopenする場合、そのオリジンが持つ最も古いハンドルを1つ
        (実機のリソースごと)dispose_resources()で解放してから新しいハンドルを
        発行する。これによりopenDevice()自体は(通常のオープン処理が成功する限り)
        常に成功を返し続けつつ、_open_devices のオリジンあたりの総数は
        上限で頭打ちになる。退去させられた古いハンドルは以後
        _get_open_device()から見えなくなり、それを使った操作は他の閉じた
        ハンドルと同じ"Invalid device handle"として安全に失敗する。"""
        try:
            origin = self._current_origin(frame_token)
            ...
            open_count_for_origin = sum(1 for info in self._open_devices.values() if info.get("origin") == origin)
            if open_count_for_origin >= self._MAX_OPEN_HANDLES_PER_ORIGIN:
                # 挿入順(Python dictは3.7+で挿入順を保持する)で最初に見つかる
                # 同一オリジンのエントリが、そのオリジンにとって最も古いハンドル。
                oldest_handle_id = next(
                    hid for hid, info in self._open_devices.items() if info.get("origin") == origin
                )
    ...
    def closeDevice(self, handle_id, frame_token=""):
        ...
        どの経路でも明示的なreturn文が無く、実際には常にNoneを返していた
        (handle_idが無効な場合に限らない、より広範な不具合だった)。
        JS側で`JSON.parse(result)`する呼び出し元(polyfill.py)にとって、
        Noneは`null`/`undefined`として届き`JSON.parse()`が例外を投げる。
        他の全Slotと同じ形の戻り値に統一する。"""
        try:
    ...
    def claimInterface(self, handle_id, interface_number, frame_token=""):
        ...
        🛡️ security_audit No.1: interface_class_for()には明示的に
        alternate_setting=0 を渡す — claim直後のinterfaceは(まだ
        selectAlternateInterface()が一度も呼ばれていない限り)常にUSB仕様上
        alternate setting 0にあるため、判定はその具体的なalternate setting
        ...
        try:
            ...
            if info is not None:
                ...
                info.setdefault("interface_alt_settings", {})[interface_number] = 0
            return _json_dumps({"success": True})
        except Exception as e:
            return _json_dumps({"success": False, "error": safe_error_str(e)})
    ...
    def selectAlternateInterface(self, handle_id, interface_number, alternate_setting, frame_token=""):
        ...
        try:
            ...
            # 「claim済みinterfaceの、今まさに選択されているalternate setting」を
            # 正しく参照できるよう、実際に切り替えが成功した後にここで記録する。
            info.setdefault("interface_alt_settings", {})[interface_number] = alternate_setting
            return _json_dumps({"success": True})
        except Exception as e:
            return _json_dumps({"success": False, "error": safe_error_str(e)})
    ...
    def resetDevice(self, handle_id, frame_token=""):
        ...
        🛡️ security_audit No.1(拡張): バスリセット後のデバイスは、リセット前とは
        異なるディスクリプタで再列挙され得る(例: DFUモードへの遷移や
        ファームウェア更新後の再起動等)。reset()の前に記録していた
        claimed_interfaces/interface_alt_settings をそのまま信用し続けると、
        「以前は無害だったinterface番号が、リセット後は実は保護対象クラスに
        変わっていた」ケースでも判定に古い前提を使い回してしまう恐れがある。
        selectConfiguration()と同様にここでも無効化し、reset後は
        claimInterface()からの明示的なやり直しを要求する(=毎回その時点の
        実際のディスクリプタに対してalternate setting 0からの判定をやり直す)。"""
        try:
            dev = self._get_open_device(handle_id, frame_token)
            if dev is None:
                return _json_dumps({"success": False, "error": "Invalid device handle"})
    ...
    def _endpoint_available_or_error(self, handle_id, dev, in_transfer, endpoint_number, required_type=None):
        ...
        🛡️ security_audit No.1: 上記の「実害は小さい」という判断は誤りだった
        — alternate setting 0が無害なクラス、alternate setting 1が保護対象
        クラス(HID等)という複合デバイスでは、claimInterface()はalternate
        setting 0だけを見て許可する一方、この関数が全alternateを横断して
        endpointを探してしまうため、selectAlternateInterface()を一度も
        呼ばずに保護対象クラス側のendpointへ実データを読み書きできてしまう
        (詳細はVULNERABILITY_REPORT.md No.1、CVE-2018-6125と同種の回避策)。
        claimInterface()/selectAlternateInterface()が更新する
        interface_alt_settings(interface番号→現在のalternate setting、
        未記録なら仕様どおりデフォルトの0)を参照し、各interfaceについて
        「claim済み」かつ「今まさに選択されているalternate setting」の
        エントリだけを対象にendpointを探すよう修正した。
        ...
        try:
            for intf in active_cfg:
                ...
                if inum not in claimed:
                    continue
                if getattr(intf, "bAlternateSetting", 0) != alt_settings.get(inum, 0):
                    continue  # claim済みだが、今は選択されていない別alternateの記述子
                for ep in intf:
