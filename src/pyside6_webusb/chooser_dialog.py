...
class _DeviceRowWidget(QWidget):
    ...
    def __init__(self, dev: dict, parent=None):
        ...
        # 🛡️ security_audit No.3: QLabelのデフォルトtextFormat(Qt::AutoText)は
        # 中身がHTMLらしく見えると自動でリッチテキストとして解釈してしまう。
        # nameはデバイス自身が名乗るproductName/manufacturerName(=USB接続できる
        # 誰でも自由に設定できる文字列)なので、"<b>Trusted Device</b>"のような
        # 細工でこのチューザーダイアログ自体の表示を偽装できてしまっていた
        # (CVE-2020-16033と同種、WebUSBのセキュリティUIスプーフィング)。
        # 明示的にPlainTextを強制し、中身に関わらず常に生の文字列として表示する。
        ...
        if detail:
            ...
            detail_label.setTextFormat(Qt.TextFormat.PlainText)  # 🛡️ 同上(念のため、こちらはVID/PIDのみで実害は無いが一律で統一)
...
class WebUsbDeviceChooserDialog(QDialog):
    ...
    def __init__(self, devices, parent=None, strings=None, origin=None, refresh_callback=None):
        ...
        heading_label.setTextFormat(Qt.TextFormat.PlainText)  # 🛡️ security_audit No.3: originを埋め込むため念のため一律適用
        heading_label.setWordWrap(True)
        heading_label.setStyleSheet("color:#e2e8f0;font-size:14px;font-weight:600;")
        layout.addWidget(heading_label)

        trust_label = QLabel(s["trust_reminder"])
        trust_label.setTextFormat(Qt.TextFormat.PlainText)  # 🛡️ security_audit No.3: 一律適用(こちらは固定文言のみ)
